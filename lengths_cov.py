...
overlap=pd.read_table(os.path.join(path, "June_1st_2021.txt"), sep='\t')

expect_combined=pd.DataFrame()
...
for i in files_orf:
    ...
    observed=overlap_sum.pivot_table(index="Sequence", columns="InORF(T/F)")
    observed.columns=["Sequence","HmerOut(BP)","HmerIn(BP)"]
    observed["Observed"]=observed["HmerOut(BP)"]/observed["HmerIn(BP)"]

    combined=pd.merge(observed,expect_combined, left_on="Sequence", right_on="Sequence")
